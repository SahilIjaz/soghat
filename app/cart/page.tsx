...
export default function CartPage() {
  ...
  // ✅ Handle checkout with email sending
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (result.success) {
        alert("✅ Order confirmed! A confirmation email has been sent to you.");
        router.push("/checkout");
      } else {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F5F5DC]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              {cartItems.map((item) => (
                <div
                  key={item.id}
                  className="rounded-xl overflow-hidden shadow-lg bg-white p-6"
                >
                  <div className="flex gap-6">
              ...
              <button
                onClick={() => router.push("/")}
                className="w-full py-3 border-2 rounded-lg font-semibold transition-all duration-300 hover:scale-105"
                style={{
                  borderColor: "#2D5541",
                  color: "#2D5541",
                  backgroundColor: "transparent",
      ...
      <footer className="mt-12 bg-[#3E2723] text-center py-6 text-[#F5F5DC] opacity-80">
        © 2025 Soghat. All rights reserved.
      </footer>
    </div>
  );
